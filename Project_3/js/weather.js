function initWeatherWidget() {
  ...
  const weatherByCity = [
    ...
    { city: "Constanta", latitude: 44.1598, longitude: 28.6348 }
  ];
  ...
  function outfitSuggestion(tempC) {
    if (tempC < 10) return "Bring a coat and scarf";
    if (tempC <= 20) return "Wear a light jacket";
    return "Use a t-shirt and sunglasses";
  }
  ...
  function renderWeatherCards(weatherRows) {
    ...
    weatherRows.forEach((row) => {
      ...
      card.innerHTML = `
        ...
        <p class="weather-updated">Updated ${row.updatedLabel}</p>
      `;
      weatherGrid.appendChild(card);
    });
  }
    ...
    card.innerHTML = `
      <h3>
        <span class="city-label">
          ...
          <span>March Snapshot</span>
        </span>
      </h3>
      <p>Sample spring conditions in Romania</p>
      <p class="weather-temp">11°C</p>
      <p class="suggestion"><i class="fa-solid fa-shirt"></i> Wear a light jacket</p>
      <p class="weather-updated">Offline mode sample</p>
    `;
    weatherGrid.appendChild(card);
  }
